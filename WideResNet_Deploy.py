...
class deployer(object):
    ...
    def cluster_filters(self, weight, n_clusters, seed):
        ...
        weight_vector_clustered = k_means_cpu(weight_vector.astype('float32'), n_clusters, seed=seed)
        ...
        weight_compress = torch.from_numpy(weight_cube_clustered.astype('float32'))
    ...
    def cluster_filters_sparsity(self, weight, n_clusters, ratio, seed):
        ...
        weight_vector_clustered = k_means_sparsity(weight_vector.astype('float32'), n_clusters, ratio=ratio, seed=seed, gpu_id=0).astype('float32')
        # unique_count = np.unique(weight_vector_clustered, axis=0, return_counts=True)[1]
        # all_count = np.sum(unique_count)
        # prob = unique_count / all_count
        # huffman_length = np.sum([- p * np.log2(p) for p in prob])
    ...
    def initialize_model(self):
        ...
        self.model = Wide_ResNet(depth=16, widen_factor=4, dropout_rate=0.3, num_classes=10)
    ...
    def test(self, model):
        ...
        print("Total {}".format(len(self.testloader.dataset)))
